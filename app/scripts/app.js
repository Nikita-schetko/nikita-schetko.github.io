            ...
            $rootScope.$on('$locationChangeStart', function (event, next, current) {
                ...
                if ($location.path() !== '/login' && !$rootScope.globals.currentUser) {
                    if (window.location.search.slice(0, 6) === '?code=' && $rootScope.logged !== true && $rootScope.requestSent !== true) {
                        ...
                        $http.post('https://le-taste.herokuapp.com/api/v1/auth/social/token_user/', { provider: 'facebook', code: $rootScope.facebookCode, redirect_uri: window.location.origin + window.location.pathname })
